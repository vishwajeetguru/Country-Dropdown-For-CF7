...
class WP_Intl_Tel_Input {
    ...
    public function register_settings() {
        ...
        register_setting( 'wp_intl_tel_input', 'wp_intl_tel_input_default_country' );
        ...
        add_settings_field(
            'wp_intl_tel_input_default_country',
            __( 'Default Country Code', 'wp-intl-tel-input' ),
            [ $this, 'settings_field_html_default_country' ],
            'wp-intl-tel-input',
            'wp_intl_tel_input_section'
        );
    }
    ...
    public function settings_field_html_default_country() {
        $value = get_option( 'wp_intl_tel_input_default_country', 'auto' );
        ?>
        <input type="text" name="wp_intl_tel_input_default_country" value="<?php echo esc_attr( $value ); ?>" />
        <p class="description"><?php _e( 'Enter the default country code (e.g., US, GB, FR) or use "auto" for automatic detection.', 'wp-intl-tel-input' ); ?></p>
        <?php
    }
    ...
    public function enqueue_scripts_styles() {
        if ( get_option( 'wp_intl_tel_input_enabled', 0 ) ) {
            ...
            $settings = [
                'defaultCountry' => get_option( 'wp_intl_tel_input_default_country', 'auto' ), // Default country code option
                'displayDialCode' => get_option( 'wp_intl_tel_input_display_dial_code', 'none' ),
                'validationErrors' => [
                    1 => __( 'Invalid country code.', 'wp-intl-tel-input' ),
                    2 => __( 'Too short.', 'wp-intl-tel-input' ),
                    3 => __( 'Too long.', 'wp-intl-tel-input' ),
                    4 => __( 'Invalid number.', 'wp-intl-tel-input' )
                ]
            ];
    ...
    public function tel_shortcode_handler( $tag ) {
        ...
        $html .= sprintf( '<input type="hidden" name="%1$s_full" />', esc_attr($tag->name) );
